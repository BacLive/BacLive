...
runGraphquestion = getBoolean("Perform Pre-Analysis of Channel Intensity Values");

// Run graph analysis if requested
	if (runGraphquestion == 1){
		AreYouSure = getBoolean("Generates a graph plotting max. intensity Z-slices over time. \n \nThis can help choosing the best channel and reference timepoints\n\n(Warning - Processing can take some time!)");
		if (AreYouSure == 1){
			...
			setBatchMode("show");		
			waitForUser("          Analysis complete          \n \n          Graph output can now be saved, closed or repositioned  \n \n          Click to continue");
		} }
...
// Option for manually-capturing Z-positions, posibility using a button menu
	...
	Dialog.addSlider("Add extra slices to Z-stack starting position", 0, 50, 20); 
	Dialog.addSlider("Add extra slices to Z-stack ending position", 0, 50, 20);
	Dialog.show();
			...
			selectImage(SourceID);
				for (TP=1; TP<=TotalTimepoints; TP++){ //timepoint loop
					...
					for (i=1; i<=TotalSlices; i++) {  //Z-slice loop
						Stack.setPosition(c, i, TP);  //The channel and start frame is determined by the user
						makeRectangle(0, 0, PXWidth, PXHeight);
