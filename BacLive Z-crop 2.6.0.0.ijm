// Set Environment and collect reconstruction info
	...
	isVirtual = is("Virtual Stack");
	SourceID = getImageID();
	SourceName = getTitle(); //needed for a specific step when source ID doesnt work
	setBatchMode("exit and display");  //Makes sure any hidden images from previous runs are displayed
...
// New position for pre-analysis option - with idea of performing analysis before making any other choices
	...
	if (runGraphquestion == 1){
		...
		probeR = probeR / (TotalChannels*20);
		...
		setBatchMode("show");		
		waitForUser("Analysis complete \n \n  Save, close or reposition graph output then click to continue");
		} 
...
// Z-stack preprocessing 
	if (isVirtual == false) {  // to avoid applying function on virtual stacks
		selectImage(SourceID);
		setBatchMode("hide");
		if (AddZSliceStart > 0) {
...
// DV Calculation Menu
	...
	if(analysisType=="Semi-Automatic Z-detection"){  //Semi-Automatic calculation -  skips if manual selection was done
		waitForUser("BacLive Semi-Automatic Z-displacement detection looks at a user-specified fluoroesence channel and start/end timepoints \nto determine an average displacement value (slices/timepoint) based on the change in z-position with the highest intensity. \n \nBefore continuing, please examine the stack and/or the analysis graph to determine the optimal channel and timepoints.");
...
// Stack Processing section
	...
	tempFolder = getDirectory("Chose an empty temporary folder for image processing");
